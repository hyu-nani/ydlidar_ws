    ...
    while (ret&&ros::ok()) {
		...
		while(true){
			...
			for(int i=0;i<500;i++){
				...
				int Xvalue = round(cos((YD_angle[i]+90)*M_PI/180.0)*YD_distance[i]*100.0/unitScale);
				int Yvalue = round(-sin((YD_angle[i]+90)*M_PI/180.0)*YD_distance[i]*100.0/unitScale);
				...
				if( (difference < 0.02) &&	(difference != 0) && (YD_distance[i] > 0.2) && (YD_distance[i] < 8))//[M]
					data_count[i]++;
				else
					data_count[i] = 0;
...
int SerialRead()
{
	...
	if(strcmp(buf,"OK")==0){
		...
		if(buf[0]=='P' && buf[1]=='o' && buf[2]=='s'){
			sscanf(buf,"Pos/%d/%d/%f",&robotX,&robotY,&robotAngle);
			printf("reading....");
		}
		return 0;
	}
