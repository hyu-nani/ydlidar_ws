    ...
    while (ret&&ros::ok()) {
		...
		while(true){
			...
			for(int i=0;i<500;i++){
				...
				int Xvalue = round(cos((YD_angle[i]+90-robotAngle)*M_PI/180.0)*YD_distance[i]*100.0/unitScale)+Xvalue;
				int Yvalue = round(-sin((YD_angle[i]+90-robotAngle)*M_PI/180.0)*YD_distance[i]*100.0/unitScale+Yvalue);
